...
dataset_name = 'bert'
saved_model_path = 'models/{}_model'.format(dataset_name.replace('/', '_'))
...
#     lines = f.readlines()
# text_test = ' '.join([str(elem) for elem in lines])
# examples = [
#     text_test
# ]
#
# reloaded_results = tf.sigmoid(reloaded_model(tf.constant(examples)))
# scores = reloaded_model.predict(examples)
# original_results = tf.sigmoid(classifier_model(tf.constant(examples)))
#
# print("The predicted category using the BERT model is %s." % (reloaded_results))
# print("The conditional probability is: %a" % (scores.max() * 100))
